...
function createWorkspace({
  ...
  async function list() {
    ...
    for (const document of [...documents()])
      result.push(snapshot(await get(document.id)));
    return {
  ...
  async function createMission(input, actor) {
    const source = await get(
      requireText(input.source_document_id, "source_document_id", 100),
    );
    if (parseContract(source))
      throw problem(422, "invalid_source", "请选择正文文档作为任务来源");
    const contract = validateMission({
      kind: "mission",
      objective: input.objective,
      source_document_id: source.id,
      status: "active",
      quiet_seconds: input.quiet_seconds ?? 8,
    });
    return snapshot(
      await create(
        {
          title: `任务 · ${source.title}`,
  ...
  async function edit(id, input, actor) {
    ...
    return lock(id, async () => {
      ...
      const content =
        typeof input.content === "string" ? input.content : d.content;
      if (content.length > 200000)
        throw problem(413, "too_large", "正文超过 200000 字符");
      const oldContract = parseContract(d),
        next = parseContract({ content });
      ...
      if (next) {
        ...
        if (source.id === d.id || parseContract(source))
          throw problem(422, "invalid_source", "任务只能关注正文文档");
      }
      return snapshot(
        await write(d, content, {
          base_version: input.base_revision,
          title: requireText(input.title ?? d.title, "title", 200),
          actor_id: actor,
        }),
      );
    });
  ...
  async function changeStatus(id, input, actor) {
    ...
    if (!["active", "paused", "completed"].includes(input.status))
      throw problem(422, "invalid_status", "无效状态");
    return lock(id, async () => {
      const d = await get(id);
      validateMission(parseContract(d));
      return snapshot(
        await write(d, updateContract(d, { status: input.status }), {
          base_version: input.base_revision,
          actor_id: actor,
  ...
  async function publishRun(input, actor) {
    ...
    return lock(missionId, async () => {
      const mission = await get(missionId),
        contract = validateMission(parseContract(mission));
      const source = await get(contract.source_document_id);
      if (
        !Number.isInteger(input.mission_revision) ||
        !Number.isInteger(input.source_revision)
      )
      ...
      const id = stableId("run", runKey),
        existing = documents().find((d) => d.id === id);
      if (existing) return { document: snapshot(existing), duplicate: true };
      if (
        contract.status !== "active" ||
        mission.revision !== input.mission_revision ||
        source.revision !== input.source_revision ||
        fingerprint(source.content) !== input.source_hash
      )
        throw problem(
          409,
          "stale_run",
          "任务或正文已变化，本次结果已丢弃，Agent 将重新观察",
        );
      const action = input.action;
      if (!["stay_silent", "propose", "blocked"].includes(action))
      ...
      if (
        ...
        quotes.length > 12 ||
        quotes.some(
          (q) => typeof q !== "string" || !q || !source.content.includes(q),
        )
      )
        throw problem(422, "invalid_evidence", "证据必须逐字引用当前正文");
      if (action === "propose") {
        requireText(input.replacement, "replacement", 200000);
        if (!quotes.length || input.replacement === source.content)
          throw problem(422, "invalid_proposal", "提案需要原文证据和实际修改");
      }
      const record = {
        kind: action === "propose" ? "proposal" : "run",
        ...
        action,
        status: action === "propose" ? "pending" : action,
        rationale: input.rationale,
        evidence_quotes: quotes,
        model: input.model || "rules",
        reasoning_effort: input.reasoning_effort || null,
        actor_id: actor,
        created_at: new Date().toISOString(),
        ...(action === "propose"
          ? { before: source.content, replacement: input.replacement }
          : {}),
      };
  ...
  async function resolve(id, input, actor) {
    if (!["accept", "reject"].includes(input.decision))
      throw problem(422, "invalid_decision", "请选择接受或拒绝");
    requireVersion(input);
    return lock(id, async () => {
      const proposal = await get(id),
        p = parseContract(proposal);
      if (p?.kind !== "proposal")
        throw problem(422, "invalid_proposal", "不是修改提案");
      if (p.status !== "pending")
        return { document: snapshot(proposal), duplicate: true };
      if (proposal.revision !== input.base_revision)
        throw problem(409, "conflict", "提案版本已变化");
      ...
      if (input.decision === "accept") {
        await lock(p.source_document_id, async () => {
          ...
          const mission = await get(p.mission_id),
            m = parseContract(mission);
          if (
            m?.status !== "active" ||
            mission.revision !== p.mission_revision ||
            source.revision !== p.source_revision ||
            fingerprint(source.content) !== p.source_hash
          ) {
            status = "conflicted";
            return;
          }
          try {
            const updated = await write(source, p.replacement, {
              base_version: source.revision,
              actor_id: actor,
              operation: "proposal_accepted",
              operation_id: id,
            });
      ...
      const updated = await write(
        proposal,
        updateContract(proposal, {
          status,
          resolved_by: actor,
          resolved_at: now,
          result_revision: sourceRevision,
        }),
        {
          base_version: proposal.revision,
          actor_id: actor,
          operation: `proposal_${status}`,
        },
      );
  ...
  async function handle(method, path, input, actor) {
    ...
    if (path === "/api/workspace/missions" && method === "POST")
      return createMission(input, actor);
    if (path === "/api/workspace/runs" && method === "POST")
      return publishRun(input, actor);
    const match = path.match(
      /^\/api\/workspace\/(documents|missions|proposals)\/([a-zA-Z0-9_-]+)$/,
    );
    if (match) {
      const [, type, id] = match;
      if (type === "documents" && method === "GET")
        return snapshot(await get(id));
      if (type === "documents" && method === "PUT")
        return edit(id, input, actor);
      if (type === "missions" && method === "PATCH")
        return changeStatus(id, input, actor);
      if (type === "proposals" && method === "POST")
        return resolve(id, input, actor);
    }
