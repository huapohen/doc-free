...
function updateContract(document, values) {
  ...
  if (!current)
    throw problem(422, "invalid_contract", "文档缺少有效的 active-agent 契约");
...
function validateMission(value) {
  if (!value || value.kind !== "mission")
    throw problem(422, "invalid_contract", "无效任务契约");
  ...
  if (!["active", "paused", "completed"].includes(value.status))
    throw problem(422, "invalid_contract", "无效任务状态");
  if (
    !Number.isInteger(value.quiet_seconds) ||
    value.quiet_seconds < 2 ||
    value.quiet_seconds > 3600
  )
...
function snapshot(document) {
  return {
    ...
    content_hash: fingerprint(document.content),
    contract: parseContract(document),
  };
}
module.exports = {
  PROTOCOL,
