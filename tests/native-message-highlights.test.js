...
test("Pin collection and room-top message are separate resources, equally managed by current Human and Agent members",async()=>{
  ...
  for(const who of [advertised.human,advertised.agent])assert.deepEqual((await advertised.call(who,advertised.base)).native_features,{message_highlights:true,message_urgencies:true,message_forward_bundles:true,message_rich_text:true,message_voice:true});
