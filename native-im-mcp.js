...
tool('im_create_topic','Explicitly create a shared topic anchored to a currently visible, unretracted message at its expected revision. A stable client_id recovers the same intent; each room/message has at most one topic. This sends no chat message and creates no reply. Ordinary replies never create topics.','POST','/rooms/:room_id/messages/:message_id/topic',{client_id:s,base_revision:n},['client_id','base_revision']);
tool('im_topics','List explicitly created topics in a current room, newest first. Hidden or retracted roots are omitted for the caller. Reads never create topics or mark messages read.','GET','/rooms/:room_id/topics',{before:n,limit:n},[],['before','limit']);
tool('im_topic','Read an explicit shared topic and its current root message using current membership and personal root visibility. Read replies through im_thread.','GET','/rooms/:room_id/topics/:topic_id');
...
tool('im_highlights','Read the independent room-top highlighted message and your own collapse state. This does not change the Pin collection or mark messages read.','GET','/rooms/:room_id/highlights');
tool('im_set_highlight','Set/replace the current room-top message with both expected top revision and source message revision, or clear it with message_id:null. Current room members have equal access regardless of Human/Agent kind.','PATCH','/rooms/:room_id/highlights',{base_revision:n,message_id:{type:['string','null']},message_revision:n},['base_revision','message_id']);
tool('im_collapse_highlight','Collapse or expand only your own current top-message revision. A newly set revision becomes visible again.','PATCH','/rooms/:room_id/highlights/preferences',{base_revision:n,collapsed:b},['base_revision','collapsed']);
tool('im_urge_message','As its author, send an in-app urgency for the current message revision to named current Human/Agent room members. Stable client_id prevents duplicate requests; no SMS, telephone, OS push or read acknowledgment is claimed.','POST','/rooms/:room_id/messages/:message_id/urgencies',{client_id:s,base_revision:n,recipient_ids:strings,channel:{enum:['in_app']}},['client_id','base_revision','recipient_ids','channel']);
tool('im_urgencies','Read urgencies you sent or were explicitly assigned during your current membership cycle. Recipients see only their own acknowledgement; senders see all selected recipients.','GET','/rooms/:room_id/urgencies',{box:{enum:['all','inbox','sent']},status:{enum:['all','pending']},limit:n,before:n},[],['box','status','limit','before']);
tool('im_read_urgency','Read a currently authorized urgency. Source edits/retractions, personal hiding and membership-cycle changes explicitly affect availability.','GET','/rooms/:room_id/urgencies/:urgency_id');
tool('im_acknowledge_urgency','Explicitly acknowledge an urgency assigned to you. This never advances message reading and does not let you acknowledge for another principal.','POST','/rooms/:room_id/urgencies/:urgency_id/ack');
...
tool('im_message_groups','Read your personal conversation filters, label memberships and navigation preferences, scoped to current rooms. The documents filter contains rooms with actually shared doc_free documents and becomes unavailable with no results when your docs app permission is denied. The labels builtin selects the union of personally labelled rooms. In layout_version 2, each personal label has parent_id labels and order places all label children immediately after the labels container. Legacy order preserves builtin relative order and child relative order; hidden groups, shortcuts and revision stay unchanged on read.','GET','/message-groups');
tool('im_configure_message_groups','Save personal sidebar order/visibility and separate mobile shortcuts with one expected revision. Messages stays first and visible. Include every builtin and personal label ID in order; the server places label children immediately after the labels container, preserving builtin and child relative order. The labels container can itself be ordered, hidden or chosen as a shortcut. message_display_rules replaces the full rule map and only filters the messages group; other groups remain complete. Any existing group except messages can use always, unread, important or never. Explicit personal-label rules override builtin rules; overlapping rules at the same level use the most restrictive mode. Important means unread valid mentions or currently actionable unacknowledged in-app urgency, never all unread messages. Omit the map to keep it unchanged; an empty map restores always.','PATCH','/message-groups',{base_revision:n,order:strings,hidden_ids:strings,shortcut_ids:strings,message_display_rules:{type:'object',additionalProperties:{type:'string',enum:['always','unread','important','never']}}},['base_revision']);
