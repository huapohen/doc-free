...
`GET /search?q=<文本>` 只搜索当前成员会话的当前消息、任务与规范文档，不搜索已撤回正文或修订历史。q 为 1–100 字符，采用大小写不敏感的子串匹配；最多 100 个结果。
...
type 为 message、document 或 task。扫描预算为 10000 条消息、2000 项任务、100 篇文档；达到预算或结果上限时 truncated 可能为 true。结果中的 content 是匹配摘要，不能作为完整工作上下文。服务端按当前权限读取规范文档，成员被移除后不会继续搜到该会话。
